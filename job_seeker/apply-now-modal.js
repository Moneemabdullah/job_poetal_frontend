document.addEventListener("DOMContentLoaded", () => {
  ...
  applyForm.addEventListener("submit", async (event) => {
    ...
    if (!jobId) {
      console.error(
        "Invalid job ID. Please make sure you are applying for a valid job."
      );
      return;
    }
    ...
    try {
      const response = await fetch(
        "https://job-protal-api.onrender.com/applications/apply/",
        {
          method: "POST",
          headers: {
      ...
      if (response.ok) {
        ...
        alert(
          "Error submitting application. Please check the console for details."
        );
      }
    } catch (error) {
      console.error(
