...
const searchJobs = () => {
  ...
  if (query) {
    ...
    fetch(
      `https://job-protal-api.onrender.com//jobs/search/?q=${encodeURIComponent(
        query
      )}`
    )
      .then((res) => res.json())
                    ...
                    <div class="bg-white p-4 rounded-lg shadow-md border-2 border-transparent hover:border-purple-500 transform transition-transform duration-300 hover:scale-105 hover:shadow-xl">
                        <div class="flex justify-between items-start">
                            ...
                            <div class="flex space-x-2">
                                <span class="text-xs bg-green-100 text-green-700 py-1 px-2 rounded">${
                                  job.employee_type
                                }</span>
                            </div>
                        </div>
                        <div class="flex justify-between items-center mt-2">
                            <p class="text-sm text-gray-600"><span class="font-semibold">Experience:</span> <span class="font-medium text-gray-800">${
                              job.experience
                            }</span></p>
                            <a href="job-details.html?id=${
                              job.id
                            }" target="_blank" class="text-indigo-500 text-sm font-medium hover:underline">Details</a>
                        </div>
                        <p class="text-sm text-gray-600"><span class="font-semibold">Description:</span> ${job.description
                          .split(" ")
                          .slice(0, 30)
                          .join(" ")}...</p>
                    </div>
...
fetch("https://job-protal-api.onrender.com//jobs/jobs/")
  ...
  .then((data) => {
    ...
    jobsContainer.innerHTML = limitedJobs
      .map(
        (job) => `
      <div class="max-w-lg lg:max-w-4xl mx-auto p-4 space-y-4">
                    <div class="bg-white p-4 rounded-lg shadow-md border-2 border-transparent hover:border-purple-500 transform transition-transform duration-300 hover:scale-105 hover:shadow-xl">
