document.addEventListener("DOMContentLoaded", function () {
  ...
  fetch("https://job-protal-api.onrender.com/jobs/categories/")
    ...
    fetch("https://job-protal-api.onrender.com/jobs/jobs/", {
      ...
      .then((response) => {
        if (response.ok) {
          ...
          throw new Error(
            "Not authorized. Please check your authentication token."
          );
        } else {
      ...
      .catch((error) => {
        ...
        if (error.message.includes("Not authorized")) {
          messageDiv.textContent =
            "Authorization error. Please check your authentication token.";
          messageDiv.className = "text-red-500";
        } else {
